...
namespace bb {
    ...
    m_num(p_layerNum), m_fromSize(p_fromSize), m_toSize(p_toSize), m_activationFunc(p_activationFunc) {
        this->m_next = nullptr;
        this->m_prev = nullptr;
        ...
        if (p_toSize > 0) {
            this->m_weights = std::make_unique<Matrix>(p_toSize, p_fromSize);
            this->m_biases = std::make_unique<Matrix>(this->m_toSize, 1);
        }
    }
    ...
    void Layer::Feedforward() {
        ...
        this->m_next->m_nodes->Mutate(this->m_activationFunc);
    }
